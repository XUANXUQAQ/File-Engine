...
char not_matched(const char* path,
	char is_ignore_case,
	char* keywords,
	char* keywords_lower_case,
	int keywords_length,
	const char* is_keyword_path);
void convert_to_pinyin(const char* chinese_str, char* output_str);
int strcmp_cuda(const char* str1, const char* str2);
char* strlwr_cuda(char* src);
char* strstr_cuda(char* s1, char* s2);
char* strrchr_cuda(const char* s, int c);
char* strcpy_cuda(char* dst, const char* src);
unsigned strlen_cuda(const char* str);
char* strcat_cuda(char* dst, char const* src);
...
void init_str_convert(void);

kernel void check(global char* str_address_ptr_array,
	global unsigned* total_num,
	global int* search_case,
	global char* is_ignore_case,
	global char* search_text,
	global char* keywords,
	global char* keywords_lower_case,
	global unsigned* keywords_length,
	global char* is_keyword_path,
	global char* output,
	global char* is_stop_collect_var)
{
	...
	init_str_convert();
	char* path = (char*)(str_address_ptr_array + thread_id * MAX_PATH_LENGTH);
	...
	if (not_matched(path, *is_ignore_case, keywords, keywords_lower_case, *keywords_length, is_keyword_path))
	{
		return;
	}
	if (*search_case == 0)
	...
	const char* is_keyword_path)
{
	for (int i = 0; i < keywords_length; ++i)
	{
		...
		if (strstr_cuda(match_str, each_keyword) == NULL)
		{
			...
			utf8_to_gbk(match_str, strlen_cuda(match_str), &gbk_buffer_ptr, &gbk_buffer_size);
			char converted_pinyin[MAX_PATH_LENGTH * 6] = { 0 };
			convert_to_pinyin(gbk_buffer, converted_pinyin);
			if (strstr_cuda(converted_pinyin, each_keyword) == NULL)
			{
				return 1;
			}
		}
...
void convert_to_pinyin(const char* chinese_str, char* output_str)
{
	const unsigned length = strlen_cuda(chinese_str);
...
unsigned short* p_utf162gbk;
unsigned short* p_gbk2utf16;
const unsigned short gbk2_utf16_2_host[] =
{
	0x8140, 0x4E02,
	0x8144, 0x4E0F,
	0x8145, 0x4E12,
...
const unsigned short gbk2_utf16_3_host[] =
{
...
void init_gbk2_utf16(void);
void init_utf162_gbk(void);
void init_gbk2_utf16_2(void);
void init_gbk2_utf16_3(void);

unsigned short gbk2_utf16_3_size;
unsigned short gbk2_utf16_2_size;
static char is_not_init = 1;
	...
	for (unsigned i_from = 0; i_from < from_len; i_from++)
	{
		if (flag)
		{
			...
			const unsigned short tmp =
				p_gbk2utf16[COMPBYTE(from[i_from - 1], from[i_from]) & ~0x8000];
